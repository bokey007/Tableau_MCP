...
const CONFIG = {
    ...
    API_URL: 'https://ambient-causes-pittsburgh-straight.trycloudflare.com/api/v1',
...
async function initializeExtension() {
    ...
    try {
        if (typeof tableau !== 'undefined' && tableau.extensions) {
            log('Tableau object found, calling initializeAsync...');

            try {
                await tableau.extensions.initializeAsync();
                log('Tableau Extensions API initialized successfully');
            } catch (initErr) {
                log('initializeAsync() rejected', initErr);
                throw new Error(`Tableau API rejection: ${initErr.message || 'Unknown error'}`);
            }
            ...
            log('Tableau object not found in window.');
            throw new Error('Tableau script potentially blocked or not loaded');
        }
    } catch (err) {
        log('Initialization failed', err);

        const attempts = parseInt(sessionStorage.getItem('initAttempts') || '0');
        if (attempts > 1) {
            log('Running in standalone mode after retries.');
            updateStatus('Standalone Mode', 'warning');
            isInitialized = false;
            setupEventListeners();

            const errorMsg = err.message || 'Unknown error';
            addMessage(`⚠️ **Limited Mode**: I couldn't connect to the Tableau Dashboard API (${errorMsg}).<br><br>If you are in Tableau, try refreshing the page or check if third-party cookies/scripts are allowed.`, 'assistant');
        } else {
            sessionStorage.setItem('initAttempts', (attempts + 1).toString());
            updateStatus('Waiting for Dashboard...', 'error');

            // Re-check after a longer delay
...
document.addEventListener('DOMContentLoaded', () => {
    log('DOM loaded, triggering initialization...');
    sessionStorage.setItem('initAttempts', '0');

    // Initial delay to allow extension window to fully settle
...
async function captureDashboardContext() {
    if (!isInitialized) {
        log('Skipping context capture: Extension not initialized');
        return;
    }

    try {
        if (!tableau || !tableau.extensions || !tableau.extensions.dashboardContent) {
            log('Tableau API objects missing during capture');
            return;
        }

        const dashboard = tableau.extensions.dashboardContent.dashboard;
        if (!dashboard) {
            log('No dashboard found in dashboardContent');
            return;
        }
        ...
        for (const worksheet of dashboard.worksheets) {
            ...
            for (const filter of filters) {
                // Skip Tableau internal Action filters
                if (filter.fieldName.startsWith('Action ') || filter.fieldName.startsWith('Action(')) {
                    continue;
                }
                // Skip computed aggregates and boolean calc fields
                if (filter.fieldName.startsWith('AGG(') || filter.fieldName.endsWith('?')) {
                    continue;
                }
        ...
        // De-duplicate filters across worksheets (only show one per field)
        const uniqueFilters = [];
        const seenFields = new Set();

        for (const f of dashboardContext.filters) {
        ...
        dashboardContext.filters = uniqueFilters;
...
async function executeDashboardAction(actionData) {
    ...
    if (typeof tableau === 'undefined' || !tableau.extensions) {
        ...
        addMessage('⚠️ Dashboard connection not available to perform this action.', 'assistant');
        return false;
    }
    ...
    try {
        ...
        switch (action) {
            case 'apply_filter':
                success = await applyFilter(dashboard, actionData);
                break;

            case 'clear_filter':
                success = await clearFilter(dashboard, actionData);
                break;

            case 'clear_all_filters':
                success = await clearAllFilters(dashboard, actionData);
                break;

            case 'set_parameter':
                success = await setParameter(dashboard, actionData);
                break;

            case 'navigate':
                success = await navigateToSheet(dashboard, actionData);
                break;
            ...
            default:
                log('⚠️ Unknown action type:', action);
                return false;
        }
        ...
        if (success) {
            ...
            log(`⚠️ Action "${action}" failed to execute or find targets`);
            return false;
        }
    } catch (err) {
        log('❌ Dashboard action failed with error:', err);
        return false;
    }
}
...
async function applyFilter(dashboard, actionData) {
    ...
    if (!worksheet) {
        ...
        worksheet = dashboard.worksheets.find(ws =>
            ws.name.toLowerCase().includes(worksheetName.toLowerCase())
        );

        if (!worksheet) {
            log('❌ Worksheet not found:', worksheetName);
            addMessage(`⚠️ Failed to apply filter: Worksheet "${worksheetName}" not found.`, 'assistant');
            return false;
        }
        log('ℹ️ Found worksheet by partial match:', worksheet.name);
    }
    ...
    try {
        ...
        log('🎯 Filter applied successfully:', { worksheet: worksheet.name, field: fieldName, values });

        // Refresh context after action
        setTimeout(() => captureDashboardContext(), 800);
        return true;
    } catch (err) {
        log('❌ Error applying filter:', err);
        addMessage(`⚠️ Error applying filter to "${fieldName}": ${err.message}`, 'assistant');
        return false;
    }
}
...
async function setParameter(dashboard, actionData) {
    ...
    try {
        log('ℹ️ Attempting to set parameter:', { name: paramName, value });
        ...
        if (!param) {
            ...
            addMessage(`⚠️ Failed to set parameter: "${paramName}" not found.`, 'assistant');
            return false;
        }

        await param.changeValueAsync(value);
        log('🎯 Parameter set successfully:', { name: param.name, value });
        ...
        addMessage(`⚠️ Error setting parameter "${paramName}": ${e.message}`, 'assistant');
        return false;
    }
}
...
async function navigateToSheet(dashboard, actionData) {
    ...
    log('ℹ️ Navigation requested for:', sheetName);
    ...
    if (worksheet) {
        // In Tableau extensions, you can't directly navigate to a sheet (tab)
        // using simple API calls if it's on a different dashboard tab.
        // However, we can notify the user.
        log('ℹ️ Navigation identified target sheet:', worksheet.name);
        addMessage(`📍 I've identified the **${worksheet.name}** worksheet. Please click on its tab to view it.`, 'assistant');
        return true;
    }

    log('❌ Sheet not found for navigation:', sheetName);
    addMessage(`⚠️ Could not find sheet "${sheetName}" to navigate to.`, 'assistant');
    return false;
}
